  ...
  try {
    ...
    if (ext === 'docx' || ext === 'doc') {
      ...
      if (text.trim()) {
        ...
        // Lấy title: tìm dòng TYPE → lấy dòng ngay sau = tên nội dung VB
        const lines = text.split('\n').map((l: string) => l.trim()).filter(Boolean);
        const TYPE_RE = /^(NGHỊ\s*QUYẾT|THÔNG\s*TƯ|NGHỊ\s*ĐỊNH|QUYẾT\s*ĐỊNH|CÔNG\s*VĂN|HƯỚNG\s*DẪN|LUẬT\b|BỘ LUẬT\b)/i;
        let titleText = name.replace(/\.[^.]+$/, '').replace(/[_-]/g, ' ');
        for (let i = 0; i < Math.min(lines.length, 20); i++) {
          if (TYPE_RE.test(lines[i])) {
            // Dòng tiếp theo_after TYPE = tên nội dung VB
            const next = (lines[i + 1] || '').trim();
            if (next.length > 5 && !TYPE_RE.test(next)) {
              titleText = next;
              break;
            }
          }
